...
class AccountList {
  ...
  factory AccountList.fromJson(Map<String, dynamic> json) {
    ...
    accountJson.map((i) => Account.fromJson(i['user'])).toList();
...
class Account {
  ...
  factory Account.fromJson(Map<String, dynamic> inputJson) {
    return Account(
        ...
        inputJson['has_anonymous_profile_picture'] as bool);
  }
  @override
  String toString() {
...
Future<Account> getAccount(String accountName) async {
  Account? account;
  Uri uriSearch = Uri.https(
      'instagram.com', '/web/search/topsearch', {'query': '$accountName'});
  http.Response searchResponse = await http.get(uriSearch);
  if (searchResponse.statusCode == 200) {
    AccountList tempUsersMap =
    AccountList.fromJson(jsonDecode(searchResponse.body));
    ...
    throw Exception(
        'Error getting data, status code: ${searchResponse.statusCode}');
  }
  return account;
}
